...
//seguridad para sqlinjecction ya que usa espacios
$candado = " ";
...
if($user == null &&  $pass = null){
    header('location:../extend/alerta.php?msj=El formato no es el correcto&c=salirs&p=salir&t=error');
}else{
    $select = $con->query(" SELECT nick, nombre, nivel, correo, foto, pass FROM usuario WHERE nick = '$usuario' AND pass= '$pass2' AND bloqueo=1");
    ...
    if ($row == 1) {
        ...
        if ($nick == $usuario && $contra == $pass2 && $nivel=='ADMINISTRADOR') {
         ...
         header('location:../extend/alerta.php?msj=Nombres de usuario o contrasea incorrectos&c=salir&p=salir&t=error');
    }
}
